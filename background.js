...
function getTabsOfWindow(window, options) {
  ...
  return window.tabs.filter((tab) => {
    return !options.ignorePinnedTabs || !tab.pinned;
  });
}
...
async function moveTabsFromCurrentDomainAction() {
  ...
  tabs.forEach((tab) => {
    chrome.tabs.move(tab.id, {
      ...
      index: currentWindow.tabs.length + tabs.length,
    });
    if (tab.pinned == true) {
...
chrome.commands.onCommand.addListener((command) => {
  switch (command) {
    case "merge_windows_and_sort_tabs":
      mergeWindowsAndSortTabsAction();
      break;
    case "merge_windows":
      mergeWindows();
    ...
    case "sort_tabs":
      sortTabs();
      break;
    case "close_tabs_from_this_domain":
      closeTabsFromCurrentDomainAction();
      break;
    case "move_tabs_from_this_domain_to_this_window":
      moveTabsFromCurrentDomainAction();
      break;
  }
});
